...
- [~] Поля Departure и Arrival при фокусе отображают список направлений
  ...
  - Оптимизации
    - [x] Результаты запроса кешируются с помощью TanStack Query (или аналога) __will not do__
    - [x] Компонент `input` протестирован (написан UI тест)

- [x] Перенести логику по отправке сетевых запросов к списку городов в `SearchForm`
